...
export async function runSync(context: CommandContext & { options: SyncOptions }): Promise<void> {
  ...
  // Status mode
  if (options.status) {
    handleStatus(logger, config);
    return;
  }

  // Sync mode
  handleSync(logger, config, options);

  // Create missing sub-docs
  if (options.createSubdocs) {
    handleSubDocs(logger, config, options);
  }
}

function handleStatus(logger: CommandContext['logger'], config: CommandContext['config']): void {
