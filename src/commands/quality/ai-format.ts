  ...
  switch (category) {
    case 'complexity':
      ...
      if (message.includes('lines')) {
        return {
          action: 'extract',
          location: `lines ${issue.line}-${(issue.line || 0) + 50}`,
...
function extractImports(content: string): string[] {
  ...
  return [...new Set(imports)].slice(0, 15); // Unique, limit to 15
}
...
export function formatAIReport(aiReport: AIReport): string {
  ...
  // Summary
  lines.push('<summary>');
  lines.push(`  <files total="${aiReport.summary.totalFiles}" with-issues="${aiReport.summary.filesWithIssues}"/>`);
  lines.push(`  <issues critical="${aiReport.summary.critical}" important="${aiReport.summary.important}" suggestions="${aiReport.summary.suggestions}"/>`);
  lines.push('</summary>');
  lines.push('');
  ...
  if (aiReport.contexts.length > 0) {
    ...
    for (const ctx of aiReport.contexts) {
      ...
      lines.push(`    <metrics lines="${ctx.metrics.lines}" functions="${ctx.metrics.functions}" avg-complexity="${ctx.metrics.complexity}"/>`);
      if (ctx.exports.length > 0) {
        lines.push(`    <exports>${ctx.exports.join(', ')}</exports>`);
      }
      if (ctx.imports.length > 0) {
        lines.push(`    <imports>${ctx.imports.slice(0, 5).join(', ')}${ctx.imports.length > 5 ? '...' : ''}</imports>`);
      }
      lines.push('  </file>');
    }
