...
export function printStatus(status: StatusResult, logger: Logger, verbose = false): void {
  ...
  if (status.git.hasChanges) {
    ...
    logger.info(`⚠️  Working tree: ${changes} changes (${status.git.staged} staged)`);
  } else {
  ...
  const typecheckOk = typecheckStatus === 'passed' || typecheckStatus === 'skipped';
  const typecheckSuffix = status.typecheck.cached ? ' (cached)' : '';
  logger.info(`${icon(typecheckOk)} Typecheck: ${typecheckStatus}${typecheckSuffix}`);
  ...
  // TODOs
  const todoIcon = status.todos.count > 50 ? '⚠️ ' : status.todos.count > 0 ? '📝' : '✅';
...
export function formatMarkdown(status: StatusResult): string {
  const healthEmoji = status.health === 'good' ? '🟢' : status.health === 'warning' ? '🟡' : '🔴';
