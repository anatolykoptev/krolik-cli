...
export function generateContextHints(
  ...
  if (domains.includes("booking")) {
    hints["concurrency"] =
      "Booking creation must prevent double-booking via Prisma $transaction.";
    hints["timezone"] =
  ...
  if (domains.includes("events")) {
    hints["tickets"] =
      "Ticket quantity must be tracked atomically. Use $transaction for purchases.";
    hints["capacity"] =
      "Check venue capacity before ticket creation. Track soldCount + reservedCount.";
    hints["relations"] =
      "Event.place is optional. TicketType.event is required (cascade delete).";
    hints["pricing"] =
      "Prices stored as integers (kopecks/cents). Convert for display only.";
  }

  if (domains.includes("crm")) {
    hints["privacy"] =
      "Customer data is sensitive. Apply proper access controls via ctx.session.";
    hints["deduplication"] =
      "Customers identified by phone+placeId. Merge duplicates on conflict.";
    hints["relations"] =
  ...
  if (domains.includes("users")) {
    hints["auth"] =
      "Use ctx.session.user for authenticated user. Never trust client-provided userId.";
    hints["relations"] =
