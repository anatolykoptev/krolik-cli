...
export function printContext(context: ContextResult, logger: Logger, verbose = false): void {
  ...
  if (context.issue) {
    ...
    if (verbose && context.issue.body) {
      const body = context.issue.body.length > 500 ? context.issue.body.slice(0, 500) + '...' : context.issue.body;
      console.log(body);
    }
  }
...
export function formatAiPrompt(data: AiContextData): string {
  const lines: string[] = [];
  const { context, config, schema, routes, files, ioSchemas, componentDetails, testDetails, hints, git, tree } = data;
  ...
  if (context.issue) {
    ...
    if (context.issue.body) {
      lines.push(`      <body>${escapeXml(truncate(context.issue.body, 1000))}</body>`);
    }
  ...
  if (git) {
    ...
    if (git.changedFiles.length > 0) {
      lines.push(`    <changed count="${git.changedFiles.length}">${git.changedFiles.slice(0, 10).join(', ')}${git.changedFiles.length > 10 ? `, +${git.changedFiles.length - 10} more` : ''}</changed>`);
    }
    if (git.stagedFiles.length > 0) {
      lines.push(`    <staged count="${git.stagedFiles.length}">${git.stagedFiles.slice(0, 5).join(', ')}</staged>`);
    }
    if (git.untrackedFiles.length > 0) {
      lines.push(`    <untracked count="${git.untrackedFiles.length}">${git.untrackedFiles.slice(0, 5).join(', ')}</untracked>`);
    }
  ...
  if (schema && schema.models.length > 0) {
    ...
    if (relevantModels.length > 0) {
      ...
      for (const model of relevantModels.slice(0, 10)) {
        ...
        const maxFields = totalFields <= 20 ? totalFields : 15;
    ...
    if (relevantEnums.length > 0) {
      ...
      if (relevantEnums.length > 15) {
        lines.push(`    <!-- ${relevantEnums.length - 15} more enums -->`);
      }
      lines.push('  </enums>');
    }
  }
  ...
  if (routes && routes.routers.length > 0) {
    const relevantRouters = filterRoutersByKeywords(routes.routers, domainKeywords);
    if (relevantRouters.length > 0) {
      lines.push('  <routes basePath="packages/api/src/routers">');
      for (const router of relevantRouters.slice(0, 8)) {
        ...
        const maxQueries = queries.length <= 8 ? queries.length : 5;
        for (const proc of queries.slice(0, maxQueries)) {
          ...
          if (proc.inputFields && proc.inputFields.length > 0) {
            ...
            lines.push(`      <query${prot}${outputAttr}>${proc.name}(${inputStr})</query>`);
          } else {
            const inputAttr = proc.inputSchema
              ...
              : proc.hasInput
                ? ' input="true"'
                : '';
            lines.push(`      <query${prot}${inputAttr}${outputAttr}>${proc.name}</query>`);
          }
        }
        if (queries.length > maxQueries) {
          lines.push(`      <queries-omitted count="${queries.length - maxQueries}"/>`);
        }

        // Show all mutations if < 8, otherwise truncate
        const maxMutations = mutations.length <= 8 ? mutations.length : 5;
        for (const proc of mutations.slice(0, maxMutations)) {
          ...
          if (proc.inputFields && proc.inputFields.length > 0) {
            const inputStr = formatInputFields(proc.inputFields);
            lines.push(`      <mutation${prot}${outputAttr}>${proc.name}(${inputStr})</mutation>`);
          } else {
            const inputAttr = proc.inputSchema
              ? ` input="${proc.inputSchema}"`
              : proc.hasInput
                ? ' input="true"'
                : '';
            lines.push(`      <mutation${prot}${inputAttr}${outputAttr}>${proc.name}</mutation>`);
          }
        }
        if (mutations.length > maxMutations) {
          lines.push(`      <mutations-omitted count="${mutations.length - maxMutations}"/>`);
        }

        lines.push('    </router>');
      }
      if (relevantRouters.length > 8) {
        lines.push(`    <!-- ${relevantRouters.length - 8} more routers in this domain -->`);
      }
      lines.push('  </routes>');
    }
  }
  ...
  if (files) {
    ...
    if (files.components.length > 0) {
      ...
      for (const comp of files.components.slice(0, 15)) {
        lines.push(`    <component>${comp}</component>`);
      }
      if (files.components.length > 15) {
        lines.push(`    <!-- +${files.components.length - 15} more components -->`);
      }
  ...
  if (context.relatedFiles.length > 0) {
    lines.push('  <files>');
    for (const file of context.relatedFiles.slice(0, 20)) {
  ...
  if (ioSchemas && ioSchemas.length > 0) {
    lines.push('  <io-schemas>');
    for (const schema of ioSchemas.slice(0, 10)) {
      lines.push(`    <${schema.type} name="${schema.name}" file="${schema.file}">`);
      ...
      if (schema.fields.length > 15) {
        lines.push(`      <!-- +${schema.fields.length - 15} more fields -->`);
      }
  ...
  if (componentDetails && componentDetails.length > 0) {
    ...
    for (const comp of componentDetails.slice(0, 8)) {
      ...
      if (comp.imports.length > 0) {
        lines.push(`      <imports>${comp.imports.slice(0, 8).join(', ')}</imports>`);
      }
    ...
    if (componentDetails.length > 8) {
      lines.push(`    <!-- +${componentDetails.length - 8} more components -->`);
    }
    lines.push('  </components-detail>');
  }
  ...
  if (testDetails && testDetails.length > 0) {
    ...
    for (const test of testDetails.slice(0, 5)) {
      lines.push(`    <test file="${test.file}">`);
      for (const desc of test.describes.slice(0, 3)) {
        lines.push(`      <describe name="${escapeXml(desc.name)}">`);
        for (const t of desc.tests.slice(0, 8)) {
          lines.push(`        <it>${escapeXml(t)}</it>`);
        }
        if (desc.tests.length > 8) {
          lines.push(`        <!-- +${desc.tests.length - 8} more tests -->`);
        }
        lines.push('      </describe>');
      }
      lines.push('    </test>');
...
function filterModelsByKeywords(
  ...
  if (allKeywords.length === 0) return models.slice(0, 20);
