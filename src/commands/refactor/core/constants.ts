/**
 * @module commands/refactor/core/constants
 * @description Shared constants for refactor command
 *
 * Clean Architecture layer definitions and namespace categorization.
 */
...
export const NAMESPACE_KEYWORDS: Record<NamespaceCategory, string[]> = {
  ...
  domain: [
    ...
    'stores', 'state', 'data', 'analysis', 'analyzer', 'checker', 'linter', 'quality', 'metrics',
  ],
  integrations: [
...
/**
 * Detect namespace category from a name
 */
export function detectCategory(name: string): NamespaceCategory {
...
/**
 * Check if a dependency is allowed between two categories
 */
export function isDependencyAllowed(
  fromCategory: NamespaceCategory,
  toCategory: NamespaceCategory,
): boolean {
  if (fromCategory === 'unknown' || toCategory === 'unknown') return true;
  if (fromCategory === toCategory) return true;
  return ALLOWED_DEPS[fromCategory].includes(toCategory);
}

/**
 * Get layer number for a category
 */
export function getLayerNumber(category: NamespaceCategory): number {
  return NAMESPACE_INFO[category].layer;
}
