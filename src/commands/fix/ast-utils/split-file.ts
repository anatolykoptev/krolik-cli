...
import type { SplitFileResult, SplitConfig } from './types';
...
const FUNCTION_PREFIXES = [
  'handle', 'create', 'get', 'set', 'is', 'has', 'can', 'should',
  'format', 'parse', 'validate', 'process', 'transform', 'build',
  'make', 'find', 'filter', 'map', 'reduce',
] as const;
...
export function splitFile(
  ...
  try {
    ...
    const fixedImportsText = isIndexFile
      ? importsText
      : fixImportsForSubdirectory(importsText);
