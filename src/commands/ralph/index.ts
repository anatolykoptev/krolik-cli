...
// ADK-based RalphOrchestrator
import { RalphOrchestrator } from '../../lib/@ralph/orchestrator';
// Model Router
import {
  estimateTotalCost,
  ...
  routeTasks,
  type TaskAttributes,
} from '../../lib/@ralph/router';
import {
  getTaskExecutionOrder,
  type PRD,
  type PRDTask,
  validatePRD,
} from '../../lib/@ralph/schemas/prd.schema';
import { cancelSession, getActiveSession, getSessionStats } from '../../lib/@storage/ralph';
import { getAttemptStats } from '../../lib/@storage/ralph/attempts';
import { getGuardrailStats } from '../../lib/@storage/ralph/guardrails';
 ...
 */
const activeOrchestrators = new Map<string, RalphOrchestrator>();

/**
 * Get orchestrator for project
 */
export function getOrchestrator(project: string): RalphOrchestrator | undefined {
 ...
 */
function setOrchestrator(project: string, orchestrator: RalphOrchestrator): void {
 ...
 */
export function getRalphStatus(projectRoot: string, prdPath?: string): RalphStatus {
...
export interface StartSessionOptions {
  ...
  wait?: boolean;
  /** Use multi-agent mode with ADK SequentialAgent/ParallelAgent */
...
export function startSessionBackground(
  ...
  if (options?.useMultiAgentMode) args.push('--use-multi-agent-mode');
...
export function formatStatusXML(status: RalphStatus): string {
