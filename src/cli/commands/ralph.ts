...
export function registerRalphCommand(program: Command): void {
  ...
  statusCmd
    ...
    .action(async (options: CommandOptions) => {
      const { getRalphStatus, formatStatusXML } = await import('../../commands/ralph');
      handleProjectOption(options);
      const ctx = await createContext(program, options);
      const status = getRalphStatus(ctx.config.projectRoot, options.prd as string | undefined);
    ...
    .action(async (options: CommandOptions) => {
      ...
      if (options.session) {
        ...
        if (existsSync(sessionLog)) {
          ...
          console.error(`<ralph-error>Session log not found: ${sessionLog}</ralph-error>`);
          process.exitCode = 1;
          return;
        }
      } else {
        ...
        if (!logFile) {
          console.error(`<ralph-error>No session logs found in ${RALPH_LOGS_DIR}/

Start a session first:
  krolik ralph start --prd .krolik/ralph/prd/your-task.prd.json</ralph-error>`);
          process.exitCode = 1;
          return;
        }
      }
  ...
  validateCmd
    ...
    .action(async (options: CommandOptions) => {
      ...
      if (!prdValidation.valid) {
        console.error(`<ralph-error>${prdValidation.error}</ralph-error>`);
        process.exitCode = 1;
        return;
      }

      // Now do heavy initialization
      const { validatePrdFile, formatValidationXML } = await import('../../commands/ralph');
      handleProjectOption(options);
      const ctx = await createContext(program, options);
  ...
  startCmd
    ...
    .action(async (options: CommandOptions) => {
      ...
      if (options.wait) {
        const { startSession } = await import('../../commands/ralph');
        handleProjectOption(options);
        const ctx = await createContext(program, options);
        ...
        if (result.success) {
          ...
          console.log(`<ralph-error>${result.error}</ralph-error>`);
          process.exitCode = 1;
        }
        return;
      }
  ...
  pauseCmd.action(async (options: CommandOptions) => {
    const { pauseActiveSession } = await import('../../commands/ralph');
    handleProjectOption(options);
    const ctx = await createContext(program, options);
  ...
  resumeCmd.action(async (options: CommandOptions) => {
    const { resumeActiveSession } = await import('../../commands/ralph');
    handleProjectOption(options);
    const ctx = await createContext(program, options);
  ...
  cancelCmd.action(async (options: CommandOptions) => {
    const { cancelActiveSession } = await import('../../commands/ralph');
    handleProjectOption(options);
    const ctx = await createContext(program, options);
  ...
  planCmd
    .option('--prd <path>', 'Path to PRD.json file')
    .action(async (options: CommandOptions) => {
      const { getRoutingPlan } = await import('../../commands/ralph');
      handleProjectOption(options);
      const ctx = await createContext(program, options);
  ...
  estimateCmd
    .option('--prd <path>', 'Path to PRD.json file')
    .action(async (options: CommandOptions) => {
      const { getCostEstimate } = await import('../../commands/ralph');
      handleProjectOption(options);
      const ctx = await createContext(program, options);
  ...
  statsCmd.action(async (options: CommandOptions) => {
    const { getRouterStats } = await import('../../commands/ralph');
    handleProjectOption(options);
    const ctx = await createContext(program, options);
