...
function rowToGuardrail(row: FelixGuardrailRow): FelixGuardrail {
  return {
    id: row.id,
    project: row.project,
...
export function getGuardrailsByProject(
  ...
  const stmt = prepareStatement<(string | number)[], FelixGuardrailRow>(db, sql);
...
export function searchGuardrails(project: string, query: string, limit = 10): FelixGuardrail[] {
  ...
  const stmt = prepareStatement<[string, string, number], FelixGuardrailRow>(db, sql);
...
export function getGuardrailStats(
  ...
  `;
  const mostUsedStmt = prepareStatement<[string], FelixGuardrailRow>(db, mostUsedSql);
