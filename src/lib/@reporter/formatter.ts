...
function formatEnrichedIssue(enriched: EnrichedIssue): string {
  ...
  return `- [ ] \`${file}${loc}\` — ${issue.message} (${effort.timeLabel})${autoTag}`;
}
...
function formatActionStep(step: ActionStep): string[] {
  ...
  // Use trimEnd() to preserve leading markers (→) while removing trailing whitespace
  if (step.snippet) {
  ...
  if (step.suggestion) {
    if (step.priority === 'critical') {
      ...
      lines.push('<details>');
      lines.push('<summary>💡 Show suggested fix</summary>');
      lines.push('');
      lines.push('```diff');
      if (step.suggestion.before) {
        lines.push(`- ${step.suggestion.before}`);
