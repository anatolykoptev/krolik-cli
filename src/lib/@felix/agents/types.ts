...
/**
 * Result of LLM selection for a task
 */
...
/**
 * Function that selects an LLM for a specific task
 */
export type LlmSelector = (task: PRDTask) => LlmSelectionResult;

/**
 * Configuration for creating agent hierarchy
 */
export interface AgentFactoryConfig {
  /** LLM instance for agents (fallback if no selector) */
  llm: BaseLlm;
  /** Function to select LLM per task (uses Model Router) */
  getLlmForTask?: LlmSelector | undefined;
