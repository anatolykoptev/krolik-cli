...
export abstract class BaseCliLlm extends BaseLlm {
  ...
  private handleCliProcess(
    ...
    const LOG_BYTES_THRESHOLD = 500; // Or every 500 bytes
    ...
    proc.stdout?.on('data', (data) => {
      ...
      // Throttled logging: log every 5 seconds or every 500 bytes
      const now = Date.now();
      const bytesSinceLog = stdout.length - lastLoggedLength;
      if (now - lastLogTime > LOG_INTERVAL_MS || bytesSinceLog > LOG_BYTES_THRESHOLD) {
        // Show preview of latest content (first 100 chars, single line)
        const preview = chunk.trim().replace(/\n/g, ' ').slice(0, 100);
        this.log(`[exec+${now - t0}ms] ${preview}${chunk.length > 100 ? '...' : ''}`);
        lastLogTime = now;
        lastLoggedLength = stdout.length;
      }
