...
import type { RalphOrchestratorConfig } from './types.js';
...
export interface Checkpoint {
  ...
  config: Partial<RalphOrchestratorConfig>;
  createdAt: string;
  updatedAt: string;
}
...
type SerializableConfig = Omit<
  Partial<RalphOrchestratorConfig>,
...
export class CheckpointManager {
  ...
  private rowToCheckpoint(row: CheckpointRow): Checkpoint {
    return {
      ...
      config: JSON.parse(row.config) as Partial<RalphOrchestratorConfig>,
   ...
   */
  private extractSerializableConfig(config: Partial<RalphOrchestratorConfig>): SerializableConfig {
    const {
      onEvent: _onEvent,
