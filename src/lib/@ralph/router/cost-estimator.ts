...
import { calculateTaskScore, estimateCost, MODEL_INFO } from './rules';
import type { CostEstimate, ModelName, TaskAttributes, TaskCostEstimate } from './types';
...
export function estimateTaskCost(task: TaskAttributes, projectPath?: string): TaskCostEstimate {
  ...
  if (projectPath) {
    ...
    if (patterns.length > 0) {
      ...
      for (const pattern of patterns) {
        const patternTier = MODEL_INFO[pattern.model].tier;
        if (patternTier !== tier) {
      ...
      escalationProbability = tier === 'cheap' ? 0.2 : tier === 'mid' ? 0.1 : 0;
    }
  } else {
  ...
  const cheapestModel: ModelName = 'flash';
  const expectedModel = suggestedModel;
  const premiumModel: ModelName = 'opus';
