...
export async function runQualityGate(
  ...
  try {
    ...
    // Extract issues from all groups in the report
    for (const group of report.groups) {
      for (const enrichedIssue of group.issues) {
        const issue = enrichedIssue.issue;

        // Map priority to severity
        let severity: QualityGateIssue['severity'];
        switch (enrichedIssue.priority) {
          case 'critical':
            severity = 'critical';
            critical++;
            break;
