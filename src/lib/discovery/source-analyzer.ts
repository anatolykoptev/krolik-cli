/**
 * @module lib/discovery/source-analyzer
 * @description Analyzes TypeScript source files to extract exported functions, classes, and their signatures
 *
 * This is domain logic for the discovery layer that uses low-level SWC parsing infrastructure.
 ...
 *
 * @deprecated Use `extractTypeString` from `@/lib/parsing/swc` directly
 */
...
function extractFromModuleItem(
  ...
  if (isExportDeclaration(item)) {
    return extractFromDeclaration(
      (item as { declaration: Node }).declaration,
      content,
      baseOffset,
      false,
    );
  ...
  if (isExportDefaultDeclaration(item)) {
    return extractFromDeclaration((item as { decl: Node }).decl, content, baseOffset, true);
  }
...
function extractFromDeclaration(
  ...
  if (isVariableDeclaration(node)) {
    return extractFromVariableDeclaration(
      node as unknown as { declarations: VariableDeclarator[] },
      content,
      baseOffset,
      isDefault,
    );
...
function extractFromVariableDeclaration(
  ...
  for (const declarator of varDecl.declarations) {
    ...
    if (isArrowFunction(init)) {
      const arrowFunc = init as unknown as {
        params?: Pattern[];
        returnType?: { typeAnnotation?: TsType };
        async?: boolean;
      };
      const returnType = extractReturnType(arrowFunc.returnType, content, baseOffset);
      members.push(
        buildFunctionMember({
          name,
          params: extractArrowParams(arrowFunc.params ?? [], content, baseOffset),
          isAsync: arrowFunc.async ?? false,
          isDefault,
          ...(returnType ? { returnType } : {}),
        }),
      );
    ...
    if (isFunctionExpression(init)) {
      ...
      const returnType = extractReturnType(funcExpr.returnType, content, baseOffset);
      members.push(
        buildFunctionMember({
          name: name !== 'anonymous' ? name : (funcExpr.identifier?.value ?? 'anonymous'),
          params: extractParams(funcExpr.params ?? [], content, baseOffset),
