...
import {
  ...
  routeTask,
  routeTasks,
} from '@/lib/@felix/router/router';
import type { ModelPreference, TaskAttributes } from '@/lib/@felix/router/types';

describe('ModelRouter', () => {
  const testProjectPath = '/tmp/test-project';

  describe('routeTask', () => {
    it('should route trivial task to free tier', () => {
      ...
      expect(decision.selectedModel).toBe('vibe-opus');
  ...
  describe('getRoutingPlanSummary', () => {
    ...
    it('should count tasks by tier', () => {
      const tasks: TaskAttributes[] = [
        { id: 'free-1', complexity: 'trivial' }, // score 10 → free tier
        { id: 'cheap-1', complexity: 'simple' }, // score 25 → cheap tier
      ...
      expect(summary.byTier.cheap).toBe(1);
