...
describe("analyzeQuality - Security", () => {
  ...
  describe("path traversal prevention", () => {
    it("rejects path traversal with ..", async () => {
      await expect(
        analyzeQuality(projectRoot, { path: "../etc/passwd" }),
      ).rejects.toThrow("escapes project root");
    });

    it("rejects path traversal from subdirectory", async () => {
      await expect(
        analyzeQuality(projectRoot, { path: "src/../../etc/passwd" }),
      ).rejects.toThrow("escapes project root");
    });
  ...
  describe("valid paths", () => {
    it("accepts relative path within project", async () => {
      ...
      await expect(
        analyzeQuality(projectRoot, { path: "src/index.ts" }),
      ).resolves.toBeDefined();
    });

    it("accepts nested relative path", async () => {
      await expect(
        analyzeQuality(projectRoot, { path: "src/lib/utils" }),
      ).resolves.toBeDefined();
    });

    it("accepts current directory", async () => {
      await expect(
        analyzeQuality(projectRoot, { path: "." }),
      ).resolves.toBeDefined();
    });
  });
});
